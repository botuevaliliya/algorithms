...
class Solution:
    def findAnagrams(self, s: str, p: str) -> List[int]:
        ...
        for i in range(len(s)-k+1):
            if set(s[i:i+k])==set(p):
                if make_dict(s[i:i+k])==p:
                    result.append(i)
        return result
    ...
    def mergeKLists(self, lists: List[Optional[ListNode]]) -> Optional[ListNode]:
        ...
        while len(lists)!=1:
            ...
            for i in range(1, len(lists), 2):
                new_list.append(mergeTwoLists(lists[i], lists[i-1]))
            if i!=len(lists)-1:
...
class Solution:
    def minWindow(self, s: str, t: str) -> str:
        ...
        #             d[s[i]] += 1
        #             k -= 1
        #             while s[i-k] not in d:
        #                 k -= 1
        #         if sum(d.values())==0:
        #             # print('result', s[i-k:i+1])
        #             if len(result)>k or result=='':
        #                 result = s[i-k:i+1]
        #     i += 1
        #     k += 1
        #     # print(i, d, k)
